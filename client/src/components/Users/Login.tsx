...
const Login = () => {
    ...
    const handlesubmitchange = (event: React.FormEvent<HTMLFormElement>) => {
        if (!showlogin) {
            login({ variables: { email: logform.email, password: logform.password } })
                ...
                .catch((error:any) => { toast.error('Tu no eres de aquí 🤔')})
                ;
        }
        else {
            signup({
    ...
    const handleResetPassword = () => {
        window.location.href = 'http://compuhenry.hopto.org:5001/recuperarcontrasena'
    }
    ...
    const responseGoogle = async(response:any) => {
        ...
        window.location.href = await'http://compuhenry.hopto.org:5000/auth/google';
